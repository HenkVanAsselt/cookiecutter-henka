...
def do_filter(filename):
    ...
    for s in lines:
        ...
        if '@file' in s:
            ...
            p = p.replace('.py', '')
            outfile.write(p)
        if ('    """' in s) or ('    r"""' in s):	# It might be the start or the end of a comment block
            if s.strip() == '"""':					# It is the termination of a comment
                pass
            else:
                s = s.replace('"""', '"""!')
        if ':param' in s:
...
# filterFile
#===============================================================================
def filterFile(filename, _out=sys.stdout):
